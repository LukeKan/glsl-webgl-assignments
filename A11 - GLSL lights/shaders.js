...
var S3 = `
	OlightDir = normalize(Pos - fs_pos);
	OlightColor = lightColor * clamp((dot(OlightDir,Dir)-cos(radians(ConeOut/2.)))/(cos(radians(ConeIn*ConeOut/2.))-cos(radians(ConeOut/2.))),0.0,1.0);
	ambientColor = ambientLightColor;
`;
...
var S5 = `
	OlightDir = normalize(Pos - fs_pos);
	OlightColor = lightColor * pow(Target/length(Pos - fs_pos),Decay) * clamp((dot(OlightDir,Dir)-cos(radians(ConeOut)))/(cos(radians(ConeIn*ConeOut))-cos(radians(ConeOut))),0.0,1.0);
...
var S6 = `
	OlightDir = normalize(Pos - fs_pos);
	OlightColor = lightColor;
	vec4 topColor = ((dot(normalVec,ADir)+1.)/2.) * ambientLightColor;
	vec4 downColor = (1.-(dot(normalVec,ADir))/2.) * ambientLightLowColor;
	ambientColor =  topColor + downColor;
`;

// Single spot light, spherical harmonics ambient
var S7 = `
	OlightDir = normalize(Pos - fs_pos);
	OlightColor = lightColor * clamp((dot(OlightDir,Dir)-cos(radians(ConeOut)))/(cos(radians(ConeIn*ConeOut))-cos(radians(ConeOut))),0.0,1.0);
